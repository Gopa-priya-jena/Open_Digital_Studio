...
{
    std::uint32_t cores;
    std::uint32_t thread_per_core;
    std::uint64_t L1i_mem, L1d_mem, L2_mem, L3_mem;
    std::uint64_t L1i_mem_total, L1d_mem_total, L2_mem_total, L3_mem_total;
    std::uint16_t L1d_assoc, L1d_LineSize, L1d_setSize;
    std::uint16_t L1i_assoc, L1i_LineSize, L1i_setSize;
    std::uint64_t L2_assoc, L2_LineSize, L2_setSize;
    std::uint64_t L3_assoc, L3_LineSize, L3_setSize;
    std::uint16_t Physical_Address_Bits;
    std::uint16_t Virtual_Address_Bits;
    std::uint64_t TLB_size;
    long double   cpu_MHz;
};
