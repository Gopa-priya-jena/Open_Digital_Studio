...
namespace OS::SHELL {
  ...
  OS_SHELL::OS_SHELL( SHELL_Type type )
  {
    ...
    if ( pipe( SHELL_Linux.WRITE ) == -1 || pipe( SHELL_Linux.READ ) == -1 || pipe( SHELL_Linux.ERROR ) == -1 )

    {
      ...
      const char *argV[] = { "bash", "--norc", "--noediting", "--noprofile", "-C", "-s", nullptr };
      posix_spawn_file_actions_t file_actions;
      posix_spawn_file_actions_init( &file_actions );

      posix_spawn_file_actions_adddup2( &file_actions, SHELL_Linux.WRITE[ 0 ], STDIN_FILENO );
      posix_spawn_file_actions_addclose( &file_actions, SHELL_Linux.WRITE[ 1 ] );

      posix_spawn_file_actions_adddup2( &file_actions, SHELL_Linux.READ[ 1 ], STDOUT_FILENO );
      posix_spawn_file_actions_addclose( &file_actions, SHELL_Linux.READ[ 0 ] );

      posix_spawn_file_actions_adddup2( &file_actions, SHELL_Linux.ERROR[ 1 ], STDERR_FILENO );
      ...
      result = fcntl( SHELL_Linux.READ[ 0 ], F_SETPIPE_SZ, OUTPUT_BUFFER_SIZE * 10 );
  ...
  void OS_SHELL::check_error()
  {
    ...
    while ( ( bytesRead = read( SHELL_Linux.ERROR[ 0 ], buffer, sizeof( buffer ) - 1 ) ) > 0 ) {
      buffer[ bytesRead ] = '\0';
  ...
  bool OS_SHELL::Get_output()
  {
    ...
    {
      output_size                  = read( SHELL_Linux.READ[ 0 ], &Output_Buffer[ 0 ], OUTPUT_BUFFER_SIZE - 1 );
