...
OS_SHELL::OS_SHELL(SHELL_Type type) {
...

  {
    ...
    int result = fcntl(SHELL_Linux.WRITE[0], F_SETPIPE_SZ, 1024);
    if (result == -1)
    ...
    if (posix_spawnp(&SHELL_Linux.processID, path, &file_actions, &attributes,
...
void OS_SHELL::check_error() {
  ...
  while ((bytesRead = read(SHELL_Linux.ERROR[0], buffer, sizeof(buffer) - 1)) >
         0) {
    buffer[bytesRead] = '\0';
...
bool OS_SHELL::Get_output() {
  ...
  {
    output_size =
        read(SHELL_Linux.READ[0], &Output_Buffer[0], OUTPUT_BUFFER_SIZE - 1);
