...
{
    FUNCTION_PROFILE_ARG((char)check, data.data());
...
bool find_substring(std::string_view                        Search_Space,
                    bool*                                   checklist,
...
std::tuple<std::vector<unsigned int>, bool> Knuth_Morris_Pratt(
    std::string_view Search_space, std::string_view element)
{
...
bool Aho_Corasick(std::string_view                        Search_Space,
                  bool*                                   checklist,
                  std::initializer_list<std::string_view> elements)
{
    return false;
